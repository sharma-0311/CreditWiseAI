...
def load_and_preprocess_data():
    """
    Loads the German Credit Data, renames columns, preprocesses,
    trains the best model, and returns the preprocessor and model.
    """
    ...
    # X_test_processed = preprocessor.transform(X_test) # Not needed for app, but good for context
    ...
    # X_test_processed_df = pd.DataFrame(X_test_processed, columns=all_feature_names, index=X_test.index) # Not needed for app

    # Handle imbalanced data with SMOTE on training data only
...
st.write("""
Welcome to the Creditworthiness Assessment System. This tool leverages a sophisticated Machine Learning model
(Random Forest Classifier) trained on the German Credit Data to predict the credit risk of an applicant.
...
if submitted:
    ...
    # First, convert user-friendly names back to original attribute codes for categorical features
    # This is crucial because the preprocessor was fitted on the original codes.
    processed_input = {}
    ...
    # Create a dummy DataFrame with all original columns from X_train for consistent structure
    # This is critical for the ColumnTransformer
    ...
    # The `original_columns` list from `load_and_preprocess_data` helps here.
